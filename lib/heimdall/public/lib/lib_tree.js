...
var LibTree = {
    ...
    displayCode: function(data, container) {
        ...
        function wrapSpan(type, value) {
            return '<span class="json-tree-code-' + type + '">' + value + '</span>';
        }
    ...
    drawTree: function(data, containerId, svg, options) {
        ...
        function pan(domNode, direction) {
            ...
            if (panTimer) {
                ...
                d3.select(domNode).
                    select('g.json-tree-node').attr("transform", "translate(" +
        ...
        var baseSvg = svg.attr("class", "json-tree-overlay")
            .call(zoomListener);
        ...
        function scrollToOffset(d) {
            ...
            $(makeID('.json-tree-xcode-selectedx')).
                removeClass('json-tree-xcode-selectedx');
            $('#' + d.data.id).addClass('json-tree-xcode-selectedx');
        }
        ...
        function update(source) {
            ...
            // Update the nodes
            node = svgGroup.selectAll("g.json-tree-node")
            ...
            nodeEnter.append("circle")
                .attr('class', 'json-tree-nodeCircle')
                .attr("r", 0)
            ...
            nodeEnter.append("text")
                ...
                .attr("dy", ".35em")
                .attr('class', 'json-tree-nodeValue')
                .attr("text-anchor", function(d) {
                    return "start";
                })
                .text(function(d) {
            ...
            // collapsed
            node.select("circle.json-tree-nodeCircle")
                .attr("r", 6.5)
                .style("fill", function(d) {
                    return d._children ? "#777" : "#fff";
                });
            ...
            node.select("rect.json-tree-nodeRect")
                .attr("x", function(d) {
            ...
            // Fade the text in
            nodeUpdate.select("text.json-tree-nodeKey")
                .style("fill-opacity", 1);
            nodeUpdate.select("text.json-tree-nodeValue")
                .style("fill-opacity", 1);
            ...
            nodeExit.select("text.json-tree-nodeKey")
                .style("fill-opacity", 0);

            nodeExit.select("text.json-tree-nodeValue")
                .style("fill-opacity", 0);

            // Update the links
            var link = svgGroup.selectAll("path.json-tree-link")
