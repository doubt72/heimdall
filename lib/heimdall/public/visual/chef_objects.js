...
function encapsulateChefObjectVisuals() {
    ...
    // Display a message while loading from the Chef server
    function loadingMessage(id) {
        var html = '<div class="' + breadcrumbsName + '">retreiving data...</div>';
        $(id).html(html);
    }

    // Create an SVG to render the visual in
    function makeSVG() {
    ...
    // Server visual/starting point for navigating all the Chef objects
    Heimdall.registerVisual('chef-server', function(id, execute, args) {
        var show_json = false;
    ...
    // Make links for breadcrumbs to appropriate visual
    ...
    // This function clusters a list of names; used for list visuals to keep the
    // (often huge) arrays from being unweildy
    var clusterMax = 20;
    ...
    // Re-encode the clusters to the final form sent to encode for the tree display
    function clusterEncode(branch, root) {
        if (typeof(branch) == 'string') {
            return branch;
        } else if (branch.constructor == Array) {
            var rc = {}
            for (var index in branch.sort(function(a, b) {return a.localeCompare(b) })) {
    ...
    // Build data for tree display of large lists
    ...
    // Function for displaying lists of given Chef object type
    function objectListScript(type) {
        var script = 'chef-' + type + '-list';

        Heimdall.registerVisual(script, function(id, execute, args) {
            loadingMessage(id);
    ...
    // Function for showing Chef objects of given type
    ...
    // These types follow the same object-list pattern
    ...
    // Helper function for converting clustered cookbooks
    ...
    Heimdall.registerVisual('chef-cookbook-list', function(id, execute, args) {
        loadingMessage(id);
        function displayData(json) {
    ...
    Heimdall.registerVisual('chef-cookbook', function(id, execute, args) {
        if (args == null ||
    ...
    Heimdall.registerVisual('chef-data-bag-item', function(id, execute, args) {
        if (args == null ||
    ...
    Heimdall.registerVisual('chef-search', function(id, execute, args) {
    ...
    // Run list items are totally different; redirect to correct object
    Heimdall.registerVisual('chef-run-list-item', function(id, execute, args) {
        var json;
        if (args == null || args.name == null) {
